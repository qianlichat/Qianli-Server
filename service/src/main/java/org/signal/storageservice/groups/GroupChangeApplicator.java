...
public class GroupChangeApplicator {
  ...
  public void applyAddMembers(GroupUser user,
                              ...
                              List<Actions.AddMemberAction> addMembers)
    ...
    if (addMembers.stream().anyMatch(member -> member.getAdded().getRole() == Role.ADMINISTRATOR) && !GroupAuth.isAdminstrator(user, group)) {
      throw new ForbiddenException();
    }
    ...
    for (Actions.AddMemberAction action : addMembers) {
      ...
      if (action.getAdded().getRole() == Role.UNKNOWN || action.getAdded().getRole() == Role.UNRECOGNIZED) {
        throw new BadRequestException();
      }
  ...
  public void applyDeleteMembers(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.DeleteMemberAction> deleteMembers)
          throws BadRequestException, ForbiddenException {
    if (deleteMembers.isEmpty()) {
      return;
    }

    if (CollectionUtil.containsDuplicates(deleteMembers.stream().map(Actions.DeleteMemberAction::getDeletedUserId).collect(Collectors.toList()))) {
      throw new BadRequestException();
    }
    ...
    Set<ByteString> deleteMemberUuids  = deleteMembers.stream().map(Actions.DeleteMemberAction::getDeletedUserId).collect(Collectors.toSet());
  ...
  public void applyModifyMemberRoles(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.ModifyMemberRoleAction> modifyMembers)
          throws BadRequestException, ForbiddenException {
    ...
    if (modifyMembers.stream().anyMatch(modify -> modify.getRole() == Role.UNKNOWN || modify.getRole() == Role.UNRECOGNIZED)) {
      throw new BadRequestException();
    }

    if (CollectionUtil.containsDuplicates(modifyMembers.stream().map(Actions.ModifyMemberRoleAction::getUserId).collect(Collectors.toList()))) {
      throw new BadRequestException();
    }
    ...
    if (!modifiedGroupBuilder.getMembersList().stream().map(Member::getUserId).collect(Collectors.toSet())
                             .containsAll(modifyMembers.stream().map(Actions.ModifyMemberRoleAction::getUserId).collect(Collectors.toList()))) {
      throw new BadRequestException();
    }
    ...
    for (Member member : currentMembership) {
      Optional<Actions.ModifyMemberRoleAction> action = modifyMembers.stream().filter(candidate -> candidate.getUserId().equals(member.getUserId())).findAny();
  ...
  public void applyModifyMemberProfileKeys(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.ModifyMemberProfileKeyAction> modifyMembers)
          throws BadRequestException, ForbiddenException {
    if (modifyMembers.isEmpty()) {
      return;
    }

    for (Actions.ModifyMemberProfileKeyAction action : modifyMembers) {
  ...
  public void applyAddMembersPendingProfileKey(GroupUser user,
                                               ...
                                               Group group,
                                               Group.Builder modifiedGroupBuilder,
                                               List<Actions.AddMemberPendingProfileKeyAction> addMembersPendingProfileKey)
          throws ForbiddenException, BadRequestException {
    ...
    if (addMembersPendingProfileKey.stream().anyMatch(pending -> pending.getAdded().getMember().getRole() == Role.ADMINISTRATOR) &&
    ...
    for (Actions.AddMemberPendingProfileKeyAction action : addMembersPendingProfileKey) {
      ...
      if (action.getAdded().getMember().getRole() == Role.UNKNOWN || action.getAdded().getMember().getRole() == Role.UNRECOGNIZED) {
        throw new BadRequestException();
      }
  ...
  public void applyDeleteMembersPendingProfileKey(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.DeleteMemberPendingProfileKeyAction> deleteMembersPendingProfileKey)
          throws BadRequestException, ForbiddenException {
    if (deleteMembersPendingProfileKey.isEmpty()) {
      return;
    }

    if (CollectionUtil.containsDuplicates(deleteMembersPendingProfileKey.stream().map(Actions.DeleteMemberPendingProfileKeyAction::getDeletedUserId).collect(Collectors.toList()))) {
      throw new BadRequestException();
    }
    ...
    Set<ByteString> deleteMembersPendingProfileKeyUuids  = deleteMembersPendingProfileKey.stream().map(Actions.DeleteMemberPendingProfileKeyAction::getDeletedUserId).collect(Collectors.toSet());
  ...
  public void applyPromoteMembersPendingProfileKey(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.PromoteMemberPendingProfileKeyAction> promoteMembersPendingProfileKey)
          throws BadRequestException, ForbiddenException {
    if (promoteMembersPendingProfileKey.isEmpty()) {
      return;
    }

    for (Actions.PromoteMemberPendingProfileKeyAction action : promoteMembersPendingProfileKey) {
  ...
  public void applyPromoteMembersPendingPniAciProfileKey(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.PromoteMemberPendingPniAciProfileKeyAction> promoteMembersPendingPniAciProfileKey)
      throws BadRequestException, ForbiddenException {
    ...
    for (Actions.PromoteMemberPendingPniAciProfileKeyAction action : promoteMembersPendingPniAciProfileKey) {
  ...
  public void applyModifyTitle(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyTitleAction modifyTitle)
          throws ForbiddenException, BadRequestException {
    if (modifyTitle == null) {
      return;
    }
  ...
  public void applyModifyDescription(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyDescriptionAction modifyDescription) {
  ...
  public void applyModifyAvatar(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyAvatarAction modifyAvatar)
          throws ForbiddenException, BadRequestException {
  ...
  public void applyModifyDisappearingMessageTimer(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyDisappearingMessageTimerAction modifyDisappearingMessageTimer)
  ...
  public void applyModifyAttributesAccess(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyAttributesAccessControlAction modifyAttributesAccess) throws ForbiddenException, BadRequestException {
  ...
  public void applyModifyMembersAccess(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyMembersAccessControlAction modifyMembersAccess) throws ForbiddenException, BadRequestException {
  ...
  public void applyModifyAddFromInviteLinkAccess(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyAddFromInviteLinkAccessControlAction action) throws ForbiddenException, BadRequestException {
  ...
  public void applyAddMembersPendingAdminApproval(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.AddMemberPendingAdminApprovalAction> actions) throws ForbiddenException {
    ...
    actions.stream().map(Actions.AddMemberPendingAdminApprovalAction::getAdded).forEach(modifiedGroupBuilder::addMembersPendingAdminApproval);
  }

  public void applyDeleteMembersPendingAdminApproval(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.DeleteMemberPendingAdminApprovalAction> actions) throws ForbiddenException {
    if (!GroupAuth.isDeleteMembersPendingAdminApprovalAllowed(user, group, actions)) {
      throw new ForbiddenException();
    }

    Set<ByteString> userIdsToRemove = actions.stream().map(Actions.DeleteMemberPendingAdminApprovalAction::getDeletedUserId).collect(Collectors.toSet());
  ...
  public void applyPromotePendingAdminApproval(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<Actions.PromoteMemberPendingAdminApprovalAction> actions) throws ForbiddenException {
    if (!GroupAuth.isPromoteMembersPendingAdminApprovalAllowed(user, group)) {
      throw new ForbiddenException();
    }

    Map<ByteString, Role> userIdToRoleMap = actions.stream().collect(Collectors.toMap(Actions.PromoteMemberPendingAdminApprovalAction::getUserId,
                                                                                             Actions.PromoteMemberPendingAdminApprovalAction::getRole,
                                                                                             (role1, role2) -> role1));
    ...
    for (Map.Entry<ByteString, Role> entry : userIdToRoleMap.entrySet()) {
  ...
  public void applyModifyInviteLinkPassword(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyInviteLinkPasswordAction modifyInviteLinkPassword) throws ForbiddenException {
  ...
  public void applyModifyAnnouncementsOnly(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, Actions.ModifyAnnouncementsOnlyAction modifyAnnouncementsOnly) throws ForbiddenException {
  ...
  public boolean applyDeleteMembersBanned(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<DeleteMemberBannedAction> actions) {
    ...
    Set<ByteString> userIdsToRemove = actions.stream().map(DeleteMemberBannedAction::getDeletedUserId).collect(Collectors.toUnmodifiableSet());
  ...
  public boolean applyAddMembersBanned(GroupUser user, byte[] inviteLinkPassword, Group group, Group.Builder modifiedGroupBuilder, List<AddMemberBannedAction> actions) {
    ...
    Set<ByteString> userIdsToAdd = actions.stream().map(AddMemberBannedAction::getAdded).map(MemberBanned::getUserId).collect(Collectors.toUnmodifiableSet());
    ...
    actions.stream().map(AddMemberBannedAction::getAdded).forEach(modifiedGroupBuilder::addMembersBanned);
    return true;
  }
}
